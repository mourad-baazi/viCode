...
```bash
# Clone it
git clone https://github.com/yourusername/CodeScopeAI.git
cd CodeScopeAI
...
# Set up your env
cp .env.example .env
# Fill in: Supabase URL/keys, Anthropic API key, GitHub token, webhook secret

# Run the SQL setup in your Supabase dashboard
# → database_setup.sql (main tables)
# → github_integration_tables.sql (GitHub integration)

# Start dev server
npm run dev
```

The dev server runs both the Vite frontend and Express backend on port 5000.
